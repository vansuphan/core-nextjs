...
const AdminBannersCreatePage = ({ user }) => {
    ...
    const saveHandler = async function() {
        ...
        let loginTimeout = setTimeout(async function() {
            let params = {
                router,
                path: `/api/v1/admin/banners?slideId=${slideId}`,
                token: user.token,
                method: 'POST',
    ...
    const header = (
        <PageHeader pretitle="Banners" title="Create" button={<BackButton />} separator={true}>
            Tạo banner
        </PageHeader>
    );
...
export default withAuth(AdminBannersCreatePage);