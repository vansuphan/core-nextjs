...
const ActivityItem = ({ children, text, desc, subtext, ...rest }) => {
  ...
  return (
    <HorizontalList {...rest}>
      <VerticalList align={VerticalListAlign.CENTER} style={{ marginRight: "10px" }}>
        <Avatar
          src={asset("/admin/images/avatar.png")}
          size={40}
          style={{ backgroundColor: DefaultStyles.colors.border }}
        />
        <ListItem size={ListItemSize.STRETCH} border={`1px solid ${DefaultStyles.colors.border}`}></ListItem>
      </VerticalList>
